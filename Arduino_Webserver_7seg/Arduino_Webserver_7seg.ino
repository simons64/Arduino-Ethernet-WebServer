        ...
        while (client.connected()) {
            if (client.available()) {   // client data available to read
                ...
                if (c == '\n' && currentLineIsBlank) {
                    ...
                    else if (StrContains(HTTP_req, "sevenSeg.js")) {
                        // send rest of HTTP header
                        Serial.println("  ---> sevenSeg.js");
                        client.println("Content-Type: application/javascript");
                        client.println("Connection: close");
                        client.println();

                        print_http_answer(sevenSeg_js);
                    }
                    else if (StrContains(HTTP_req, "demo.js")) {
                        // send rest of HTTP header
                        Serial.println("  ---> demo.js");
                        client.println("Content-Type: application/javascript");
                        client.println("Connection: close");
                        client.println();

                        print_http_answer(demo_js);
                    }
                    else if (StrContains(HTTP_req, "prettify.css")) {
                        // send rest of HTTP header
                        Serial.println("  ---> prettify.css");
                        client.println("Content-Type: text/css");
                        client.println("Connection: close");
                        client.println();

                        print_http_answer(prettify_css);
                    }
